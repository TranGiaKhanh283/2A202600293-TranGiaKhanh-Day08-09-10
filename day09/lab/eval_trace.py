...
def run_test_questions(questions_file: str = "data/test_questions.json") -> list:
    ...
    for i, q in enumerate(questions, 1):
        ...
        try:
            result = run_graph(question_text, question_id=q_id)
...
def compare_single_vs_multi(
    ...
    default_path = os.path.join(os.path.dirname(__file__), "data", "day08_baseline_metrics.json")
    path = day08_results_file or default_path
    day08_baseline: dict = {}
    ...
    d9_conf = multi_metrics.get("avg_confidence", 0)
    d8_conf = float(day08_baseline.get("avg_faithfulness_5", 0) or 0) / 5.0
    d9_lat = multi_metrics.get("avg_latency_ms", 0)
    d8_lat = int(day08_baseline.get("avg_latency_ms_estimate", 0) or 0)
    ...
    comparison = {
        ...
        "analysis": {
            "routing_visibility": "Day 09 có route_reason + workers_called trong trace; Day 08 không có.",
            "latency_delta_ms": (d9_lat - d8_lat) if d8_lat else None,
            "confidence_proxy_delta": round(d9_conf - d8_conf, 3) if d8_conf else None,
            "debuggability": "Multi-agent: test từng worker + MCP; single-agent: một khối RAG.",
            "mcp_benefit": "Policy worker gọi search_kb/get_ticket_info qua MCP mock, không hard-code Chroma trong policy_tool.",
        },
    }
