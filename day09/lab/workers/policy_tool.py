...
def run(state: dict) -> dict:
    ...
    try:
        # Step 1: Evidence chỉ qua MCP search_kb (không gọi ChromaDB trực tiếp trong worker này)
        top_k = int(state.get("retrieval_top_k", 8))
        if not chunks:
            mcp_result = _call_mcp_tool("search_kb", {"query": task, "top_k": top_k})
            state["mcp_tools_used"].append(mcp_result)
            state["history"].append(
                f"[{WORKER_NAME}] MCP search_kb top_k={top_k} (policy path — no direct Chroma)"
            )
            out = mcp_result.get("output") or {}
            if out.get("chunks"):
                chunks = out["chunks"]
